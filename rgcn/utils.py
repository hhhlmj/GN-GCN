...
def build_sub_graph(num_nodes, num_rels, triples, use_cuda, gpu):
    ...
    # 节点特征
    norm = comp_deg_norm(g)
    node_id = torch.arange(num_nodes).view(-1, 1).to(g.device)
    norm = norm.view(-1, 1).to(g.device)

    g.ndata.update({'id': node_id, 'norm': norm})
    g.apply_edges(lambda edges: {'norm': edges.dst['norm'] * edges.src['norm']})

    # 边类型特征
    rel_tensor = torch.LongTensor(rel_all).to(g.device)
    ...
    g.uniq_r = uniq_r
    g.r_len = r_len
    g.r_to_e = torch.from_numpy(np.array(r_to_e)).to(g.device) if use_cuda else r_to_e
